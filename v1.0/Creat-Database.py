...
def create_database(database_name):
    if does_database_exists(database_name):
        print(termcolor.colored(f"Database named with \"{database_name}\" is already exist", "red", "on_black"))
        return False
    else:
        mycursor.execute(f"CREATE DATABASE {database_name}")
        print(termcolor.colored(f"The database named \"{database_name}\" was created", "green", "on_black"))
        return True
...
def create_table(database_name,table_name):
    if does_table_exists(database_name, table_name):
        print(termcolor.colored(f"Table named with \"{table_name}\" is already exist in \"{database_name}\"", "red", "on_black"))
        return False
    else:
        mycursor.execute(f"USE {database_name}")
        mycursor.execute(f"CREATE TABLE {table_name} (car_brand varchar(255), car_name varchar(255), car_model varchar(255), car_options varchar(255), car_price varchar(255), car_worked varchar(255), car_color varchar(255), car_condition varchar(255), car_vin varchar(255))")
        print(termcolor.colored(f"The table named \"{table_name}\" was created in \"{database_name}\"", "green", "on_black"))
        return True
...
while True:
    use_default_name = input("Do you want to use default name for database? (y for Yes / n for No): ")
...
while True:
    use_default_name = input("Do you want to use default name for table in your database? (y for Yes / n for No): ")
    if use_default_name.casefold() == "y":
        table_name = "used_cars"
        create_table(database_name, table_name)
        break
    elif use_default_name.casefold() == "n":
        table_name = input("Please enter table name you wish to create for in your database: ")
...
with open(f"{file_location_string}"+"Database.py", "w") as Database:
    ...
    Database.write("\n")
    Database.write("#Description: This file created by Create-Database.py to be used in Fetch_Used_Cars.py")
    Database.write("\n")
